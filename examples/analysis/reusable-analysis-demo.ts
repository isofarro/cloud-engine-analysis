...
async function reusableAnalysisDemo() {
  ...
  try {
    ...
    console.log(
      'Demonstrating analysis of multiple positions with the same task instance\n'
    );
    ...
    for (let i = 0; i < positions.length; i++) {
      ...
      console.log(
        `   ✓ Best line: ${result.pvs[0]?.split(' ').slice(0, 4).join(' ')}...`
      );
      console.log('');
    }

    console.log(
      '✅ Successfully analyzed multiple positions with the same task instance!'
    );
    console.log(
      '🔄 This demonstrates the reusable nature of the new interface.'
    );
  } catch (error) {
