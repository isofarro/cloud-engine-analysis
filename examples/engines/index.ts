...
export async function runAllExamples() {
  ...
  try {
    ...
    console.log(
      'Note: Remote engine example requires proper SSH configuration'
    );
    console.log(
      'Skipping remote engine example. Run it separately if needed.\n'
    );
...
export { engineServiceExample, localEngineExample, remoteEngineExample };
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (args.length === 0) {
    ...
    console.log('Usage:');
    console.log(
      '  npm run examples              # Run safe examples (service + local)'
    );
    ...
    console.log('');
    console.log(
      'Make sure to configure engine-config.json before running examples.'
    );
    process.exit(0);
  }
