...
function loadLocalEngineConfig() {
  ...
  if (!localEngine) {
    ...
    console.error(
      'Please add a local engine configuration to engine-config.json'
    );
    process.exit(1);
  }
  ...
  try {
    ...
    if (result.lines.length > 1) {
      ...
      result.lines.forEach((line, index) => {
        console.log(
          `${index + 1}. ${line.moves.slice(0, 3).join(' ')} (${line.evaluation}cp)`
        );
      });
    }
    ...
    if (
      ...
      console.error(
        '1. Install Stockfish: brew install stockfish (macOS) or apt-get install stockfish (Ubuntu)'
      );
      console.error(
        '2. Update enginePath in engine-config.json to point to your engine binary'
      );
