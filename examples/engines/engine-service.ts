...
function loadEngineConfig(): { engines: EngineDefinition[], serviceConfig: any } {
...
export async function engineServiceExample() {
    ...
    try {
        ...
        console.log('Registered engines:', engineService.getRegisteredEngines().map(e => e.name));

        // Get a local engine instance (assuming first engine is local)
        const localEngine = engines.find(e => e.type === 'local');
        if (!localEngine) {
            console.error('No local engine configured. Please add a local engine to engine-config.json');
            return;
        }
        ...
        analysis.lines.forEach((line, index) => {
            console.log(`Line ${index + 1}: ${line.moves.slice(0, 5).join(' ')} (${line.evaluation}cp)`);
        });
        ...
        if (error.message.includes('ENOENT') || error.message.includes('not found')) {
            ...
            console.error('Make sure the enginePath points to a valid chess engine binary.');
        }
    } finally {
        // Clean up
