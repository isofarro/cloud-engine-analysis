...
function loadRemoteEngineConfig() {
  ...
  if (!remoteEngine) {
    ...
    console.error(
      'Please add a remote engine configuration to engine-config.json'
    );
    process.exit(1);
  }
...
export async function remoteEngineExample() {
  ...
  try {
    ...
    console.log(
      `Nodes searched: ${analysis.engineInfo.nodes?.toLocaleString() || 'N/A'}`
    );
    ...
    const tacticalPosition =
      'r1bqr1k1/ppp2ppp/2n2n2/2bpp3/2B1P3/3P1N2/PPP2PPP/RNBQ1RK1 w - - 0 8';
    ...
    console.log(
      `Nodes per second: ${perfResult.engineInfo.nps?.toLocaleString() || 'N/A'}`
    );
