...
export class EngineService {
    ...
    async getEngine(engineId: string): Promise<ChessEngine> {
        ...
        if (this._engines.size >= this._config.maxEngines!) {
            throw new Error(`Maximum number of engines (${this._config.maxEngines}) reached`);
        }
    ...
    async disconnectAllEngines(): Promise<void> {
        const disconnectPromises = Array.from(this._engines.keys())
            .map(engineId => this._disconnectEngine(engineId));
    ...
    private async _performHealthChecks(): Promise<void> {
        const healthPromises = Array.from(this._engines.entries())
            .map(async ([engineId, engine]) => {
                try {
                    const isHealthy = await engine.healthCheck();
                    if (!isHealthy) {
                        console.warn(`Engine ${engineId} failed health check, disconnecting...`);
                        await this._disconnectEngine(engineId);
                    }
                } catch (error) {
